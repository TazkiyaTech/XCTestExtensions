...
* [XCUIElementQuery.assertCount(equals:)](Sources/XCTestExtensions/XCUIElementQueryExtension.swift) – Asserts that the receiving element's `count` property equals the given value. This is a nicer alternative to writing `XCTAssertEqual(myElementQuery.count, 1)`. Instead, you can write `myElementQuery.assertCount(1)`.
